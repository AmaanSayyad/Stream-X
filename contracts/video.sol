...
contract Ballot {
   ...
   event StreamCreated(uint index);
    ...
    function endStream(uint stream_id) public {
        require(streams[stream_id].owner == msg.sender, "Only owner can stop a stream");
        streams[stream_id].hasEnded = true;
    }
